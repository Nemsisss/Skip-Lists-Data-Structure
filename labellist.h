...
private:
    ...
    /**
     * @brief unlabels the given node from the given index of labels_ 
     * 
     * @param node the msgNode to be unlabelled
     * @param index the index which the function unlabels the given node 
     */
    void unlabelByIndex ( MsgNode* node, size_t index );
    /**
     * @brief finds the node that comes right before the given node in the all label 
     * 
     * @param node the msgNode whosee before node is to be found
     * @param index the index of the label the function is to search for the before node
     * @return MsgNode* the node that comes right before the given node in the all label
     */
    MsgNode* findBeforeNode (const MsgNode* node, size_t index);

    /**
     * @brief resizes the given node's prev_/next_/labelled_
     * 
     * @param node the msgNode to be resized
     * @param index the new size to which the given node's prev_/next_/labelled_ are to be resized
     */
    void resizeNode (MsgNode* node, size_t newSize);
